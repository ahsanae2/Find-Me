...
import { selectContacts, updateContacts , selectLists, updateAdd, selectNotify,selectName } from '../redux/slices/dataSlice';
    ...
    <View style={tailwind`flex-1 bg-white`}>
      ...
      <TextInput
                    ...
                    />
                     ...
                     {notify?.map(user => (
                    <TouchableOpacity onPress={async () => {await sendPushNotification() }} style={tailwind`bg-blue-500 w-16 h-16 rounded-full items-center justify-center shadow absolute bottom-4 right-4 z-50`}>
                    <Ionicons name="arrow-forward" size={24} color="white" />
            </TouchableOpacity>
            ))}
                     <ScrollView style={tailwind`flex-1`}>
                    {notify?.filter((e)=>e.name.toLowerCase().includes(lowerCaseSongSearch)).map(user => (
                        <View key={user.number} style={tailwind`px-3 py-3 flex-row items-center`}>
                        <MaterialIcons name="person" size={34} color="#5B79E0" style={[tailwind` w-12 h-12 rounded-full`,{backgroundColor:'#e6e6e6',padding:7}]} />
                            ...
                            <View>
                                <BouncyCheckbox fillColor="#5B79E0" isChecked={false} onPress={() => handleSelect(user)} />
                            </View>
                        </View>
                    ))}
                </ScrollView>
