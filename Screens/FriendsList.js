...
const ChatScreen = ({ route }) => {
  ...
  useEffect(() => {
    ...
    onSnapshot(unsubscribe, (snapshot) => {
      ...
      console.log(datas);
      const chatForThisUser = datas.filter((x) =>
        x?.users?.map((x) => x.number).includes(user.phoneNumber)
      );
  ...
  return (
    <View style={styles.container}>
      ...
      {loading && <ActivityIndicator size="small" color="#5B79E0" />}
      ...
      <View style={tailwind`px-3 flex-row items-center my-2`}>
        <View style={tailwind`flex-1 relative `}>
          <TouchableOpacity
            onPress={openEmoji}
            style={tailwind`absolute left-3 z-20 items-center justify-center h-full`}
          >
        ...
        <TouchableOpacity
          disabled={!message}
          onPress={sendMessage}
          style={tailwind`ml-2 w-12 h-12 rounded-full bg-blue-500 items-center justify-center`}
        >
