...
import {View, SafeAreaView, StyleSheet,FlatList,TouchableOpacity, Alert} from 'react-native';
import {
  Avatar,
  Title,
  Caption,
...
import { collection, getFirestore,doc, deleteDoc, getDoc, getDocs, where, query } from "firebase/firestore"; 
...
const handleDelete = () => {
  ...
  const deleteChat = async () => {
      ...
      await  deleteDoc(doc(chatRef, user.phoneNumber)).then(() => {
          setLoading(false);
          firebase.auth().signOut().then(() =>{
          })
      }).catch((error) => {
    ...
    <Loader visible={loading} />
      ...
      <View style={styles.userInfoSection}>
        ...
        <View style={styles.row}>
          ...
          <Text style={{color:"#000", marginLeft: 20,fontSize: 18,}}>{user.phoneNumber}</Text>
        </View>
        <View style={styles.row}>
          <Icon  color="#5B79E0" size={20}/>
          <Text style={{color:"#777777", marginLeft: 20,fontSize: 18,}}></Text>
        </View>
         <View style={styles.row}>
         <Icon name="account-group-outline" color="#5B79E0" size={22}/>
         
          <Text style={{color:"#000", marginLeft: 20,fontSize: 18,}}>Groups: {groups}</Text> 
        </View> 
      </View>
      ...
      <View style={[tailwind`py-5 px-7 flex-row justify-between items-center` ]}>
            ...
            <TouchableOpacity  onPress={deleteLeaveChat}
               style={tailwind`bg-blue-500 w-32  h-12 rounded-lg items-center justify-center   `}
               >
           <Icon  style={styles.appButtonText}  size={40} backgroundColor="#5B79E0" color='black'>Exit</Icon>
     </TouchableOpacity>
